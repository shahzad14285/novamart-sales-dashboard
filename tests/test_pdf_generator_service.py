...
@pytest.fixture
def executive_report(sample_df: pd.DataFrame) -> Report:
    ...
    return ReportingService().generate_report(ReportType.EXECUTIVE, context)
...
def test_generates_valid_pdf_bytes(executive_report: Report) -> None:
    service = PDFGeneratorService()
    result = service.generate_pdf(executive_report)
...
def test_cover_page_contains_title_period_and_audience(executive_report: Report) -> None:
    service = PDFGeneratorService()
    result = service.generate_pdf(executive_report)
...
def test_all_sections_are_rendered_in_order(executive_report: Report) -> None:
    service = PDFGeneratorService()
    result = service.generate_pdf(executive_report)
    text = _pdf_text(result.content)
...
def test_kpi_section_shows_labels_and_formatted_values_not_raw_icons(executive_report: Report) -> None:
    service = PDFGeneratorService()
    result = service.generate_pdf(executive_report)
    text = _pdf_text(result.content)
...
def test_business_insights_section_shows_key_metrics(executive_report: Report) -> None:
    service = PDFGeneratorService()
    result = service.generate_pdf(executive_report)
    text = _pdf_text(result.content)
...
def test_regional_and_product_summaries_show_currency_formatted_values(executive_report: Report) -> None:
    service = PDFGeneratorService()
    result = service.generate_pdf(executive_report)
    text = _pdf_text(result.content)
...
def test_footer_and_page_numbers_present_by_default(executive_report: Report) -> None:
    service = PDFGeneratorService()
    result = service.generate_pdf(executive_report)
    text = _pdf_text(result.content)
...
def test_custom_branding_changes_company_name_and_footer(executive_report: Report) -> None:
    service = PDFGeneratorService()
    branding = PDFBrandingConfig(company_name="NovaMart Retail Group", footer_text="Custom Footer")
    result = service.generate_pdf(executive_report, branding=branding)
    text = _pdf_text(result.content)
...
def test_show_cover_page_false_skips_cover_page(executive_report: Report) -> None:
    service = PDFGeneratorService()
    with_cover = service.generate_pdf(executive_report)
    without_cover = service.generate_pdf(executive_report, branding=PDFBrandingConfig(show_cover_page=False))
...
def test_show_page_numbers_false_omits_page_number_text(executive_report: Report) -> None:
    service = PDFGeneratorService()
    result = service.generate_pdf(executive_report, branding=PDFBrandingConfig(show_page_numbers=False))
    text = _pdf_text(result.content)
...
def test_watermark_text_appears_on_every_page(executive_report: Report) -> None:
    service = PDFGeneratorService()
    result = service.generate_pdf(executive_report, branding=PDFBrandingConfig(watermark_text="DRAFT"))
    text = _pdf_text(result.content)
...
def test_table_of_contents_lists_every_section_with_a_page_number(executive_report: Report) -> None:
    service = PDFGeneratorService()
    result = service.generate_pdf(executive_report, branding=PDFBrandingConfig(show_table_of_contents=True))
    text = _pdf_text(result.content)
...
def test_invalid_branding_type_raises() -> None:
    service = PDFGeneratorService()
    with pytest.raises(InvalidBrandingConfigError):
        service.generate_pdf(Report(report_type="x", metadata=ReportMetadata("x", datetime.now(timezone.utc)), sections=()), branding="not a config")  # type: ignore[arg-type]
...
def test_empty_report_produces_valid_pdf_instead_of_raising() -> None:
    reporting_service = ReportingService()
    reporting_service.define_report(ReportType.WEEKLY, (SectionSpec("business_insights", required=False),))
    empty_report = reporting_service.generate_report(ReportType.WEEKLY, ReportContext())
    assert empty_report.is_empty()

    service = PDFGeneratorService()
    result = service.generate_pdf(empty_report)
...
def test_section_with_empty_mapping_content_renders_placeholder_text() -> None:
    report = Report(
        report_type="custom",
...
def test_invalid_report_type_raises() -> None:
    ...
    with pytest.raises(InvalidReportInputError):
        service.generate_pdf({"not": "a report"})  # type: ignore[arg-type]
...
def test_register_content_renderer_handles_a_brand_new_content_type_without_class_changes() -> None:
...
def test_unregistered_content_type_falls_back_to_plain_text_instead_of_raising() -> None:
    report = Report(
        report_type="custom",
        metadata=ReportMetadata(title="Custom", generated_at=datetime(2026, 1, 1, tzinfo=timezone.utc)),
...
def test_renderer_exception_is_wrapped_in_pdf_rendering_error() -> None:
...
def test_shared_instance_generates_a_pdf(executive_report: Report) -> None:
    result = sales_pdf_generator_service.generate_pdf(executive_report)
    assert isinstance(result, PDFResult)
    assert result.content[:4] == b"%PDF"
