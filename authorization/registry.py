"""Authorization Provider Registry for the NovaMart Permission-Based
...
``monitoring.registry.MonitoringProviderRegistry`` above all (identical
shape and API), plus ``tenancy.registry.TenantRegistry`` -- so swapping
the active identity backend is the same kind of operation as swapping
the active monitoring backend: one ``register()`` call with a plain
object, never a code change to
:class:`~authorization.service.AuthorizationService` or to any business
service or UI call site it serves.
"""

from __future__ import annotations

from authorization.exceptions import NoActiveProviderError, ProviderNotRegisteredError
...
class AuthorizationProviderRegistry:
    """A registry of every authorization provider known to the platform, plus which one is active.

    Exactly one provider is "active" at a time -- the one
    :class:`~authorization.service.AuthorizationService` uses by
    ...
    Example:
        ...
        >>> registry.register("memory", InMemoryAuthorizationProvider(), make_active=True)
        >>> registry.active_name
        'memory'

        # Adding a future identity backend later, without touching this
        # class or AuthorizationService:
        >>> registry.register("azure_ad", AzureADAuthorizationProvider(...))
        >>> registry.set_active("azure_ad")
    """
    ...
    def register(self, name: str, provider: AuthorizationProvider, *, make_active: bool = False) -> None:
        """Register (or replace) a provider under ``name``.

        Args:
            name: A short, stable key for this provider (e.g.
                ``"memory"``, ``"database"``, ``"azure_ad"``,
                ``"okta"``, ``"auth0"``).
    ...
    def get(self, name: str) -> AuthorizationProvider:
    ...
    def get_active(self) -> AuthorizationProvider:
        """Return the currently active provider.

        Returns:
            The active :class:`~authorization.provider.AuthorizationProvider`.
...
# authorization backend the moment this module is imported, with zero
